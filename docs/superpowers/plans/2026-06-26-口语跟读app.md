...
- 单元测试命令统一：
  `xcodebuild test -scheme Listen -destination 'platform=iOS Simulator,name=iPhone 15' -only-testing:ListenTests/<TestClass>`
  （若本机无 iPhone 15 模拟器，改成 `xcrun simctl list devices` 里存在的机型名。）
